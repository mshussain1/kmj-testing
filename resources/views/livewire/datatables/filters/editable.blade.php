<div x-data class="flex flex-col p-1 mb-2">
    <x-form-input x-ref="input" type="text"
    ...
    <div class="flex flex-wrap max-w-48 space-x-1">
        @foreach ($this->activeTextFilters[$index] ?? [] as $key => $value)
            <button wire:click="removeTextFilter('{{ $index }}', '{{ $key }}')"
                class="m-1 pl-1 flex items-center uppercase tracking-wide bg-green-500 text-white hover:bg-red-600 rounded-full focus:outline-none text-xs space-x-1">
